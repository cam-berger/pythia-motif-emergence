...
| 3 — Push past replication / 1B scale-extension | 11–13 | **✓ §H3-scale complete — gate FAILS, REGR pattern; supplementary evidence is novel** (pre-registered §H3-scale 5-leg conjunctive gate FAILS at A.i + A.iii + B.i + B.ii; A.ii passes at the maximum: `P(μ_si^1B < μ_si^410m) = 1.000` over 1000 paired bootstraps. **Empirical signature beyond the matched REGR headline: count axis saturates between 410m and 1B (max=3 at both); timing axis accelerates monotonically with scale; H1-C ordering breaks at 1B because S-inhibition emerges before successor.** Per §H3-scale-9, the matched REGR headline stands and the supplementary evidence is documented in `notebooks/h1c_ordering_test.ipynb` §H3-scale extension; §S-5c-style supplementary-acceptance amendment deferred to writeup phase. Anchor inspections at Pythia-1B step143000 PASS on all gates; no TOOLING-pattern detector breakage at d_model = 2048. 40 cells × 1B-deduped × 3 motifs; new `phase3_1b_*` parquets; `notebooks/{induction,successor,s_inhibition}_full_sweep_1b.ipynb` + cross-cutting extensions to `h1c_ordering_test.ipynb`, `motif_structural_reuse.ipynb`, `motif_attention_inspection.ipynb`.) |
