...
| 3b — Head-count-axis scaling (§H4-scaling, registered) | 12–13 | **⏳ in progress — registered, prefetch running** (§H4-scaling pre-registered before any 2.8B compute. Two-leg conjunctive gate at Pythia-2.8B-deduped, 1024 heads = next ~3× tier from 410m's 384: (A.timing) `P(μ_si^2.8B < μ_si^410m) ≥ 0.95` over B=1000 paired bootstraps; (A.count) `max_count_si^2.8B ≥ 5`. 5-pattern failure-mode taxonomy with priority `TOOLING > NEITHER > COUNT-ONLY > TIMING-ONLY > PASS`. 40-cell prefetch (~700 GB safetensors-only) running in background; anchor + sweep + verdict pending user trigger.) |
