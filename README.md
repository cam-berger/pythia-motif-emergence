...
Tigges et al. (NeurIPS 2024) characterized IOI and greater-than circuit emergence across Pythia checkpoints but did not study successor heads or copy-suppression. McDougall et al. (BlackboxNLP 2024) explicitly named *"do copy-suppression heads exist in Pythia and Llama?"* as future work. The infrastructure (`curt-tigges/circuits-over-time`, `hannamw/EAP-IG`) exists; the experimental design — three tracks: a pre-registered emergence-ordering test at three Pythia sizes (§H1-C / §H2-5, gate passed at p = 0.00463), a pre-registered head-count-axis scaling test at Pythia-2.8B (§H4-supersede, PASS), and a pre-registered inference-time causal-dependence ablation across the 410m, 1B, and 2.8B anchors (§H5-causal / §H5-causal-2 / §H5-causal-3; NULL × NULL at 410m and 2.8B, with the 1B Metric B MIXED case treated as a narrow-architecture readout-specificity artifact) — is the contribution.

The registered ordering gate (induction → successor → suppression) passes across the three small sizes, but with important scale-dependence: the cleanest per-size confirmation is at 410m, while smaller-size legs contain censoring and marginal fits. The result supports temporal emergence ordering, while the causal-dependence and structural-reuse tracks test whether that temporal order is also an architectural causal chain.
...
| 4 — Writeup | 14–16 | **WRITEUP.md three-track narrative complete; paper draft v0.2 in progress** — Tracks 1 + 2 + 3 all PASS or NULL-as-target. Paper-narrative re-organization in `WRITEUP.md`; HYPOTHESIS.md remains canonical pre-reg record. |
