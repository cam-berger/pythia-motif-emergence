"""Phase 2 successor sweep — 40 cells × 3 sizes (HYPOTHESIS.md §H2-1, §H2-6).

Per cell: run successor_screen with `return_per_prompt=True` so the
bootstrap post-processing (§H2-2) can resample prompts. Save per-cell
row in long-format parquet plus per-cell `.npz` of per-prompt scores.

Outputs:
  - data/exploration/phase2_successor_sweep.parquet — long format
    (size, step, layer, head, motif, score) where score = lift_dla
  - data/exploration/phase2_successor_per_prompt/{size}_step{step}.npz —
...
OUT_PARQUET = (
    REPO_ROOT / "data" / "exploration" / "phase2_successor_sweep.parquet"
)
OUT_NPZ_DIR = REPO_ROOT / "data" / "exploration" / "phase2_successor_per_prompt"
