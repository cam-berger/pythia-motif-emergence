"""Pythia-410M @ step143000 successor anchor inspection (Phase 1.4 §SU-5).

Runs the §SU-1b lift-form successor detector on the pre-registration
anchor checkpoint. Applies the two §SU-5 anchor gates:

  Numerical gate:    ≥1 Pythia-410M head clears τ_lift = 0.13496 (locked
                     §SU-tau).
  Qualitative gate:  the top candidate has cross-category breadth — lift
                     positive across ≥3 of 4 categories (not concentrated
                     in a single category, which would indicate sequence
                     memorization rather than abstract successor).

Per §SU-5 + Q9-(b), failure of either gate does NOT block the 18-cell
sweep — it documents a Path-C-style finding. The sweep proceeds
unconditionally.

Outputs:
  - data/exploration/successor_pythia_410m_anchor.parquet (long format)
  - data/exploration/successor_pythia_410m_anchor_per_head.npz (raw)
"""
...
OUT_AGG = (
    ...
    / "data"
    / "exploration"
    / "successor_pythia_410m_anchor.parquet"
)
OUT_RAW = (
    REPO_ROOT
    / "data"
    / "exploration"
    / "successor_pythia_410m_anchor_per_head.npz"
)
OUT_PROMPTS = REPO_ROOT / "data" / "prompts" / "successor_prompts_pythia.tsv"

SIZE = "410m"
STEP = 143000
TAU_LIFT = 0.13496  # locked §SU-tau (from GPT-2 small validation, 95th-pct of pooled per-head lifts)
BATCH_SIZE = 8
...
def main() -> None:
    ...
    print("\n=== HYPOTHESIS.md §SU-5 anchor gates ===")
    flat = result.lift_dla.flatten()
    ranking = torch.argsort(flat, descending=True)

    print("Top 10 Pythia-410M candidates by lift:")
    for rank in range(10):
        ...
        print(
            f"  #{rank + 1:2d}: L{L:>2d}H{H:<2d}  lift={lift:+.4f}{flag}"
        )
    ...
    for cat in CATEGORIES:
        ...
        print(f"  {cat:>10s}: lift = {lift_v:+.4f}  (real {real_v:+.4f}, null {null_v:+.4f})  {marker}")
    ...
    print(
        f"Per Q9-(b): regardless of anchor gate outcomes, the 18-cell sweep "
        f"proceeds unconditionally."
    )
