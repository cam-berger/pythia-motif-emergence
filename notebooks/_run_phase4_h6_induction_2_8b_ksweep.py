"""§H6-causal-ksweep-2.8b — induction K-dose-response at Pythia-2.8B step 143000.

Pre-registered in HYPOTHESIS.md §H6-causal-ksweep-2.8b. Mirrors the 410M
K-sweep, scaled to 2.8B's 34-head induction-detected population. Locked
K ∈ {5, 12, 18, 25, 34}; K=34 expected to yield K_eff=32 after 2-head NM
exclusion (L13H9 + L17H12), flagged in the verdict row.

Outputs:
  - phase4_h6_induction_2_8b_ksweep.parquet
  - phase4_h6_induction_2_8b_ksweep_verdict.parquet
  - phase4_h6_induction_2_8b_ksweep.log
"""
...
OUT_PER_PROMPT = EXPL / "phase4_h6_induction_2_8b_ksweep.parquet"
OUT_VERDICT = EXPL / "phase4_h6_induction_2_8b_ksweep_verdict.parquet"
LOG_PATH = EXPL / "phase4_h6_induction_2_8b_ksweep.log"
...
K_VALUES = [5, 12, 18, 25, 34]

# §H5-causal-3-2.8b locked receiver sets (inherited verbatim)
LOCKED_NM_HEADS = [(11, 29), (17, 12), (22, 31), (13, 9)]
LOCKED_SI_SENDERS = [(11, 29), (11, 5), (13, 9)]
LOCKED_SUC_RECEIVERS = [(15, 14), (28, 17), (27, 13), (13, 10), (29, 28)]

# Total induction-detected at 2.8B step143000 (for pct_ablated reporting)
N_INDUCTION_DETECTED = 34
...
def main() -> None:
    ...
    log("=== §H6-causal-ksweep-2.8b — Pythia-2.8B induction K-dose-response ===")
    log(f"K values: {K_VALUES}")
    t_start = time.time()

    # Combined induction sweep (phase2 + phase3_1b + phase4_2_8b)
    df_ind = pd.concat([
        read_long(EXPL / "phase2_induction_sweep.parquet"),
        read_long(EXPL / "phase3_1b_induction_sweep.parquet"),
        read_long(EXPL / "phase4_2_8b_induction_sweep.parquet"),
    ...
    for K in K_VALUES:
        ...
        ctrl_set, ctrl_bw = select_ctrl_induction(
            ...
            nm_set=LOCKED_NM_HEADS, si_set=LOCKED_SI_SENDERS,
            suc_receivers_set=LOCKED_SUC_RECEIVERS,
            k=len(ind_set),  # match ctrl size to actual K_eff (handles exhaustion case)
        )
        ...
        per_K[K] = dict(ind_set=ind_set, ind_lh=ind_lh,
                         ...
                         excluded=excluded, k_effective=len(ind_set))
        log(f"\nK_target={K}, K_eff={len(ind_set)}:")
        log(f"  ind  set: {', '.join(f'L{l}H{h}' for l, h, _ in ind_set)}")
        log(f"  ctrl set (bw={ctrl_bw:.3f}): {', '.join(f'L{l}H{h}' for l, h, _ in ctrl_set)}")
        if excluded:
    ...
    PER_CONDITION_BUDGET_MIN = 30  # 2.8B anchor screens are fast

    for K in K_VALUES:
        log(f"\n========== K_target = {K} (K_eff={per_K[K]['k_effective']}) ==========")
        K_t0 = time.time()
        ...
        verdict_rows.append(dict(
            K_target=K,
            K_effective=per_K[K]["k_effective"],
            n_induction_pop=N_INDUCTION_DETECTED,
            pct_ablated=100.0 * per_K[K]["k_effective"] / N_INDUCTION_DETECTED,
            structural_caveat_k_exhausted=(per_K[K]["k_effective"] < K),
