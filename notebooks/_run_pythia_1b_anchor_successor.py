"""Pythia-1B @ step143000 successor anchor inspection (§H3-scale-5).

Runs the §SU-1b lift-form successor detector on Pythia-1B-deduped @ step143000.
Applies the two §SU-5 anchor gates verbatim (per §H3-scale-5):

  Numerical gate:    ≥1 Pythia-1B head clears τ_lift = 0.13496 (locked
                     §SU-tau).
  Qualitative gate:  the top candidate has cross-category breadth — lift
                     positive across ≥3 of 4 categories.
...
Outputs:
  - data/exploration/successor_pythia_1b_anchor.parquet (long format)
  - data/exploration/successor_pythia_1b_anchor_per_head.npz (raw)
"""
...
OUT_AGG = (
    ...
    / "data"
    / "exploration"
    / "successor_pythia_1b_anchor.parquet"
)
OUT_RAW = (
    REPO_ROOT
    / "data"
    / "exploration"
    / "successor_pythia_1b_anchor_per_head.npz"
)
OUT_PROMPTS = REPO_ROOT / "data" / "prompts" / "successor_prompts_pythia_1b.tsv"

SIZE = "1b"
STEP = 143000
TAU_LIFT = 0.13496  # locked §SU-tau
BATCH_SIZE = 4  # smaller batch for d_model=2048 MPS safety
...
def main() -> None:
    ...
    print("\n=== HYPOTHESIS.md §H3-scale-5 successor anchor gates ===")
    flat = result.lift_dla.flatten()
    ranking = torch.argsort(flat, descending=True)

    print("Top 10 Pythia-1B candidates by lift:")
