...
def build() -> nbf.NotebookNode:
    ...
    nb.cells = [
        ...
        # ============================================================
        # §H5-causal-3-record: 1B post-data canonicalization
        # ============================================================
        md(
            "---\n"
            "\n"
            ...
            "\n"
            "- **Pythia-1B step143000** (head-count tier 128, head-count regression per §H4-1). "
            "Compute ran 2026-05-07 on a feature worktree before the corresponding amendment "
            "was canonicalized; HYPOTHESIS.md §H5-causal-3-record records the verdicts as-found "
            "and acknowledges the pre-reg-discipline gap.\n"
            "- **Pythia-2.8B step143000** (head-count tier 1024). HYPOTHESIS.md §H5-causal-3-2.8b "
            "registered the protocols and locked sets before any 2.8B ablation compute. "
            "Locked suc = `[(15,14),(28,17),(27,13),(13,10),(29,28)]`; SI senders = "
            "`[(11,29),(11,5),(13,9)]`; NMs = `[(11,29),(17,12),(22,31),(13,9)]`; "
            "ctrl procedure-locked (bracket-widening, seed=0, NM-excluded).\n"
            "\n"
            "All thresholds (DEP_THRESHOLD = 0.5, NULL_BAND = ±0.20 for Metric A; "
            "[0.8, 1.2] for Metric B), bootstrap parameters (B = 200, seed = 1), "
            "ablation method (mean-ablation on hook_z per length group), and verdict "
            "taxonomies are inherited verbatim from §H5-causal / §H5-causal-2."
        ),
        md(
        ...
        md(
            "**Reading the 1B verdict.** Metric A NULL replicates the 410m converging-evidence "
            "result. Metric B returns MIXED with ratio_suc ≈ ratio_ctrl ≈ 0.79 — both ablations "
            "drop the logit-diff by ~21%, but neither is suc-specific. This is best interpreted "
            "as a **narrow-architecture readout-specificity artifact**: at 1B's 128-head "
            "architecture (head-count regression per §H4-1), the IOI logit-diff readout itself "
            "becomes generically ablation-sensitive — it can no longer distinguish suc-specific "
            "from control ablations. The Metric A path-patching scalar, which routes through "
            "specific NM heads, remains specific and returns NULL. The 2.8B result (next section) "
            "confirms this interpretation by recovering NULL on both metrics at head-count "
            "tier 1024."
        ),
        # ============================================================
        # §H5-causal-3-2.8b: 2.8B pre-data verdict
        # ============================================================
        md(
            "---\n"
            "\n"
