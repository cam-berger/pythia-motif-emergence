"""Pythia-2.8B @ step143000 S-inhibition anchor inspection (§H4-4).

Inherits §S-8 anchor gates verbatim:
  Numerical:    ≥1 head clears τ_strict = 0.0372 (locked §S-tau).
  Mechanistic:  ≥2/4 of Pythia-2.8B's NMs (component-DLA top-4) show
                positive Δ along path candidate → NM on the top candidate.

Per §H4-4: regardless of anchor outcome, the 40-cell sweep proceeds
unconditionally. Anchor result is documentation.

BATCH_SIZE=10 per §H4-7 (halved from 1B's 25 for path-patching memory
pressure at d_model=2560 × 1024 heads).

Outputs:
  - data/exploration/s_inhibition_pythia_2_8b_anchor.parquet (long format)
  - data/exploration/s_inhibition_pythia_2_8b_anchor_per_nm.npz (raw)
"""
...
OUT_AGG = (
    ...
    / "data"
    / "exploration"
    / "s_inhibition_pythia_2_8b_anchor.parquet"
)
OUT_RAW = (
    REPO_ROOT
    / "data"
    / "exploration"
    / "s_inhibition_pythia_2_8b_anchor_per_nm.npz"
)
...
def main() -> None:
    ...
    print("\n=== HYPOTHESIS.md §H4-4 S-inhibition anchor gates ===")
    flat = result.delta_h.flatten()
    ranking = torch.argsort(flat, descending=True)
    top10_idx = ranking[:10]

    print("Top 10 Pythia-2.8B @ step143000 candidates by Δ_h:")
    ...
    print(
        f"\nMechanistic gate (≥2 of 4 Pythia-2.8B NMs positive on top candidate): "
