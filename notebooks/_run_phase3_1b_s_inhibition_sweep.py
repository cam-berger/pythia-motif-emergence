"""Phase 3 1B S-inhibition sweep — 40 cells × Pythia-1B-deduped (HYPOTHESIS.md §H3-scale).
...
IOI prompts. Save per-cell row in long-format parquet plus per-cell `.npz`
of per-prompt path-patching deltas.

This is the longest sweep at 1B (path-patching is per-(sender, NM) per cell);
estimated ~90 min wall time based on the §H3-scale-5 anchor extrapolation
(2.3 min/cell × 40 cells).

Outputs:
  - data/exploration/phase3_1b_s_inhibition_sweep.parquet — long format
    (size, step, layer, head, motif, score) where score = delta_h
  - data/exploration/phase3_1b_s_inhibition_per_prompt/1b_step{step}.npz —
...
OUT_PARQUET = (
    REPO_ROOT / "data" / "exploration" / "phase3_1b_s_inhibition_sweep.parquet"
)
OUT_NPZ_DIR = REPO_ROOT / "data" / "exploration" / "phase3_1b_s_inhibition_per_prompt"
...
BATCH_SIZE = 25  # halved from Phase 2 (50) for d_model=2048 MPS safety (matches anchor)
