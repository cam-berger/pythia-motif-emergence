"""Pythia-1B @ step143000 induction anchor inspection (§H3-scale-5).

Runs the Olsson prefix-matching detector on Pythia-1B-deduped @ step143000.
Single anchor gate per HYPOTHESIS.md §H3-scale-5 (induction anchor):
...
Outputs:
  - data/exploration/induction_pythia_1b_anchor.parquet (long format)
  - data/exploration/induction_pythia_1b_anchor_per_seq.npz (raw)
"""
...
OUT_AGG = (
    ...
    / "data"
    / "exploration"
    / "induction_pythia_1b_anchor.parquet"
)
OUT_RAW = (
    REPO_ROOT
    / "data"
    / "exploration"
    / "induction_pythia_1b_anchor_per_seq.npz"
)
...
def main() -> None:
    ...
    # §H3-scale-5 gate
    print("\n=== HYPOTHESIS.md §H3-scale-5 induction anchor gate ===")
    flat = result.scores.flatten()
    sorted_idx = flat.argsort(descending=True)

    print("Top 10 Pythia-1B candidates by prefix-matching score:")
