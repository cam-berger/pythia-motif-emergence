...
def build() -> nbf.NotebookNode:
    ...
    nb["cells"] = [
        ...
        code(
            ...
            "ax.plot(ours_mld['step'], ours_mld['value'], marker='o', color='C2')\n"
        ...
        code(
            ...
            "    ax.hist(sub['logit_diff'], bins=30, color='C0', alpha=0.8)\n"
        ...
        code(
            ...
            "for kind, color in [('ABBA', 'C0'), ('BABA', 'C1')]:\n"
        ...
        md(
            "## Verdict\n"
            "\n"
            "If the gate passed: Phase 1.2 is complete; Phase 1.3 (S-inhibition "
            "detector validation) can begin. If the gate failed: per the locked "
            "debugging procedure (NOTES.md), check (a) tokenizer version (`-v0` "
            "fallback), (b) MPS-vs-CPU numerical drift on a 5-prompt subset, and "
            "(c) deduped-vs-no-dropout variant — switch the model template in "
            "`pythia_loader.py` if the failure is concentrated near step10000 "
            "where Tigges' published value is most distinctive."
        ),
    ]
    return nb
