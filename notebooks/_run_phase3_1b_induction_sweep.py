"""Phase 3 1B induction sweep — 40 cells × Pythia-1B-deduped (HYPOTHESIS.md §H3-scale).

Per cell: run Olsson prefix-matching with `return_per_sequence=True` so the
bootstrap post-processing (§H2-2 inherited per §H3-scale-10) can resample
sequences. Save per-cell row in long-format parquet plus per-cell `.npz` of
per-sequence scores for cached bootstrap input.

Outputs:
  - data/exploration/phase3_1b_induction_sweep.parquet — long format
    (size, step, layer, head, motif, score)
  - data/exploration/phase3_1b_induction_per_seq/1b_step{step}.npz —
...
OUT_PARQUET = (
    REPO_ROOT / "data" / "exploration" / "phase3_1b_induction_sweep.parquet"
)
OUT_NPZ_DIR = REPO_ROOT / "data" / "exploration" / "phase3_1b_induction_per_seq"
...
def main() -> None:
    ...
    print(
        f"Wrote {len(CHECKPOINTS)} per-seq .npz files in "
        f"{OUT_NPZ_DIR.relative_to(REPO_ROOT)}/"
    )
