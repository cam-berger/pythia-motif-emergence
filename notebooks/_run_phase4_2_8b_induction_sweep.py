"""Phase 4 2.8B induction sweep — 40 cells × Pythia-2.8B-deduped (HYPOTHESIS.md §H4-scaling).

Per cell: run Olsson prefix-matching with `return_per_sequence=True` so the
bootstrap post-processing (§H2-2 inherited per §H4-10) can resample sequences.
Save per-cell row in long-format parquet plus per-cell `.npz` of per-sequence
scores for cached bootstrap input.

Outputs:
  - data/exploration/phase4_2_8b_induction_sweep.parquet
  - data/exploration/phase4_2_8b_induction_per_seq/2.8b_step{step}.npz
"""
...
OUT_PARQUET = (
    REPO_ROOT / "data" / "exploration" / "phase4_2_8b_induction_sweep.parquet"
)
OUT_NPZ_DIR = REPO_ROOT / "data" / "exploration" / "phase4_2_8b_induction_per_seq"
