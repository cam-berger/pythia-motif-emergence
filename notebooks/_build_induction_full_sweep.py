...
def build() -> nbf.NotebookNode:
    ...
    nb["cells"] = [
        ...
        md(
            "## Animation: induction prefix-matching across (layer, head) over training (all 40 frames)\n"
            "\n"
            "The static grid above shows every 4th checkpoint (10 of 40). For "
            "the full evolution dynamics, the cell below builds a 40-frame "
            "animation across all 5 sizes (one panel per Pythia size, frame per "
            "checkpoint, ~3 fps → ~13 s wall time).\n"
            "\n"
            "**Outputs:**\n"
            "- Inline HTML5 player (visible in JupyterLab + GitHub-rendered .ipynb).\n"
            "- Standalone GIF at `notebooks/figures/induction_evolution.gif` for "
            "  sharing / paper-figure use.\n"
            "\n"
            "Each frame applies a shared color scale (0 → max prefix-matching across "
            "the full sweep) so density growth across sizes is visually comparable. "
            "The 1B panel's narrower head-count dimension (8 cols vs 410m's 16, "
            "2.8b's 32) is rendered with `aspect='auto'` so all panels appear at "
            "the same display size despite different (layer, head) shapes."
        ),
        code(
            "import matplotlib.animation as animation\n"
            "from IPython.display import HTML\n"
            "\n"
            "# Pre-compute (n_layers, n_heads) heatmap per (size, step). 5 sizes × 40 steps = 200 grids.\n"
            "heatmaps_by_size = {}\n"
            "for size in SIZES:\n"
            "    sub = df[df['size'] == size]\n"
            "    n_layers = int(sub['layer'].max() + 1)\n"
            ...
            "            grid[int(r['layer']), int(r['head'])] = r['score']\n"
            "        frames.append(grid)\n"
            "    heatmaps_by_size[size] = frames\n"
            "vmax = float(df['score'].max())\n"
            "\n"
            "fig, axes = plt.subplots(1, len(SIZES), figsize=(4 * len(SIZES), 5))\n"
            "ims = []\n"
            "for ax, size in zip(axes, SIZES):\n"
            "    initial = heatmaps_by_size[size][0]\n"
            "    im = ax.imshow(initial, vmin=0, vmax=vmax, cmap='viridis', aspect='auto')\n"
            "    ax.set_title(f'Pythia-{size}', fontsize=10)\n"
            "    ax.set_xlabel('head')\n"
            "    ax.set_ylabel('layer')\n"
            "    ims.append(im)\n"
            "title = fig.suptitle(f'Induction prefix-matching across (layer, head) — step{STEPS[0]}', y=1.0)\n"
            "fig.colorbar(ims[-1], ax=axes, fraction=0.012, pad=0.02, label='prefix-matching')\n"
            "\n"
            "def update(frame_idx):\n"
            "    step = STEPS[frame_idx]\n"
            "    for im, size in zip(ims, SIZES):\n"
            "        im.set_array(heatmaps_by_size[size][frame_idx])\n"
            "    title.set_text(\n"
            "        f'Induction prefix-matching across (layer, head) — step{step} ({frame_idx+1}/{len(STEPS)})'\n"
            "    )\n"
            "    return ims + [title]\n"
            "\n"
            ...
            "out_gif = out_dir / 'induction_evolution.gif'\n"
            "ani.save(out_gif, writer=animation.PillowWriter(fps=3))\n"
            "print(f'Wrote {out_gif.relative_to(REPO)}')\n"
            "plt.close(fig)  # prevent the static initial-frame figure from rendering separately\n"
            "HTML(ani.to_jshtml())"
        ),
        md(
        ...
        code(
            ...
            "ax.plot(fit_x, fit_y, '--', color='gray', alpha=0.8, label=fit_eq, zorder=2)\n"
            ...
            "plt.show()\n"
            "\n"
            "print(f'Linear best fit:  y = {slope:.4f}x + {intercept:.2f}    R² = {r_squared:.3f}')\n"
            "print(f'Interpretation: each additional total attention head is fit as +{slope:.3f} additional induction heads.')\n"
            "print()\n"
