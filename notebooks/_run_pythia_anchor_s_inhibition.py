"""Pythia-410M @ step143000 S-inhibition anchor inspection (Phase 1.3 §S-8).

Runs the locked detector on the pre-registration anchor checkpoint with the
GPT-NeoX-tokenized N=200 IOI prompt set from Phase 1.2. Applies the two Q8
anchor gates:

  Numerical gate:    ≥1 head clears τ_strict = 0.0372 (locked §S-tau).
  Mechanistic gate:  for the top candidate, ≥2 of 4 Pythia NMs (component-
                     DLA top-4 on Pythia-410M @ step143000) show positive Δ.

Per Q9-(b), failure of either gate does NOT block the 18-cell sweep — it
documents a Path-C-style finding. The sweep proceeds unconditionally.

Outputs:
  - data/exploration/s_inhibition_pythia_410m_anchor.parquet (long format)
  - data/exploration/s_inhibition_pythia_410m_anchor_per_nm.npz (raw)
"""
...
OUT_AGG = (
    ...
    / "data"
    / "exploration"
    / "s_inhibition_pythia_410m_anchor.parquet"
)
OUT_RAW = (
    REPO_ROOT
    / "data"
    / "exploration"
    / "s_inhibition_pythia_410m_anchor_per_nm.npz"
)
...
def main() -> None:
    ...
    # Mechanistic gate: top candidate's per-NM contributions
    ...
    print(
        f"\nMechanistic gate (≥2 of 4 Pythia NMs positive on top candidate): "
